...
/**
 * \brief Constructor.
 * \param p The position of this control point. It will be assigned to the
 * input and output directions too.
 */
template <typename C, typename Traits>
claw::math::curve<C, Traits>::control_point::control_point(
...
claw::math::curve<C, Traits>::control_point::control_point(
  ...
  : m_position(p)
  , m_input_direction(input_direction)
 ...
 */
template <typename C, typename Traits>
const typename claw::math::curve<
    C, Traits>::section::resolved_point::coordinate_type&
 ...
 */
template <typename C, typename Traits>
double claw::math::curve<C, Traits>::section::resolved_point::get_date() const
{
  return m_date;
}
...
claw::math::curve<C, Traits>::section::get_point_at(double t) const
{
  ...
  const value_type x(
      evaluate(t, traits_type::get_x(m_origin->get_position()),
  ...
  const value_type y(
      evaluate(t, traits_type::get_y(m_origin->get_position()),
...
{
  const value_type dx
      = evaluate_derived(t, traits_type::get_x(m_origin->get_position()),
                         traits_type::get_x(m_origin->get_output_direction()),
                         traits_type::get_x(m_end->get_input_direction()),
                         traits_type::get_x(m_end->get_position()));

  const value_type dy
      = evaluate_derived(t, traits_type::get_y(m_origin->get_position()),
                         traits_type::get_y(m_origin->get_output_direction()),
                         traits_type::get_y(m_end->get_input_direction()),
                         traits_type::get_y(m_end->get_position()));
...
claw::math::curve<C, Traits>::section::get_point_at_x(value_type x,
                                                      bool off_domain) const
{
  ...
  const std::vector<double> roots(
      get_roots(x, traits_type::get_x(m_origin->get_position()),
                traits_type::get_x(m_origin->get_output_direction()),
                traits_type::get_x(m_end->get_input_direction()),
                traits_type::get_x(m_end->get_position())));

  for(std::size_t i = 0; i != roots.size(); ++i)
    result.push_back(resolved_point(get_point_at(roots[i]), *this, roots[i]));

  ensure_ends_in_points(result, (x == m_origin->get_position().x),
                        (x == m_end->get_position().x));
...
claw::math::curve<C, Traits>::section::evaluate(double t, value_type origin,
                                                ...
                                                value_type end) const
{
  ...
  return origin * dt * dt * dt + 3 * output_direction * t * dt * dt
         + 3 * input_direction * t * t * dt + end * t * t * t;
}
 ...
 * points. If one end of the curve is expected to be in the resolved point,
 * then this procedure replaces the nearest resolved point by a resolved point
 * on the adequate end.
 *
 ...
 */
template <typename C, typename Traits>
std::vector<double> claw::math::curve<C, Traits>::section::get_roots(
    value_type x, value_type origin, value_type output_direction,
    value_type input_direction, value_type end) const
{
  const value_type a(-origin + 3 * output_direction - 3 * input_direction
                     + end);
 ...
 */
template <typename C, typename Traits>
std::vector<double> claw::math::curve<C, Traits>::section::get_roots_degree_2(
 ...
 */
template <typename C, typename Traits>
std::vector<double> claw::math::curve<C, Traits>::section::get_roots_degree_3(
    value_type a, value_type b, value_type c, value_type d) const
{
  ...
  const value_type q((b / (27.0 * a)) * ((2.0 * b * b) / (a * a) - 9.0 * c / a)
                     + d / a);
  ...
  if(delta == 0)
    ...
    {
      result.push_back(
          std::pow(((-q + std::sqrt(delta)) / 2.0), 1.L / 3.L)
          + std::pow(((-q - std::sqrt(delta)) / 2.0) - b / (3.0 * a),
                     1.L / 3.L));
    }
  else
...
template <typename C, typename Traits>
std::vector<typename claw::math::curve<C, Traits>::section::resolved_point>
claw::math::curve<C, Traits>::get_point_at_x(value_type x,
                                             bool off_domain) const
{
