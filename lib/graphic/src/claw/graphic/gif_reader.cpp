...
claw::graphic::gif::reader::input_buffer::input_buffer(std::istream& is,
  ...
  , m_pending_bits(0)
  , m_pending_end(0)
  , m_initial_code_size(code_size)
  , m_code_size(m_initial_code_size + 1)
  , m_code_limit(1 << m_code_size)
{
  m_input.read(reinterpret_cast<char*>(&m_next_data_length),
...
claw::graphic::gif::reader::output_buffer::output_buffer(
  ...
  : m_palette(p)
  , m_id(id)
  , m_transparent_color_index(transparent_color_index)
  , m_output(output)
  , m_x(0)
  , m_y(0)
  , m_interlace_pass(0)
 ...
 * \param f The stream from which we read the data (gif file format).
 */
claw::graphic::gif::reader::reader(frame_list& frames, std::istream& f)
  : m_image(NULL)
{
  load(f);
  frames = m_frame;
 ...
 * constructor.
 */
claw::graphic::gif::reader::reader(image& img, frame_list& frames,
                                   std::istream& f)
  : m_image(&img)
{
  load(f);
...
{
  std::for_each(m_frame.begin(), m_frame.end(),
                claw::delete_function<frame*>());
  m_frame.clear();
}
 ...
 */
void claw::graphic::gif::reader::read_data(std::istream& f, reader_info& info)
{
  u_int_8 code;
 ...
 * \param info Some global data needed when reading the file.
 */
void claw::graphic::gif::reader::read_frame(std::istream& f, reader_info& info)
{
